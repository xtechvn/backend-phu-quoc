    ...
    public class ContractPayDAL : GenericService<ContractPay>
    {
        ...
        public List<ContractPayDetail> GetByContractPayIds(List<int> contractPayIds)
        {
            try
            {
                ...
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var contractPays = _DbContext.ContractPayDetail.AsNoTracking().Where(x => contractPayIds.Contains(x.PayId)).ToList();
        ...
        public List<ContractPayDetail> GetByContractDataIds(List<long> dataIds)
        {
            try
            {
                ...
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var detail = _DbContext.ContractPayDetail.AsNoTracking().Where(x => x.DataId != null && dataIds.Contains(x.DataId.Value)).ToList();
                    if (detail != null)
                    {
                        return detail;
                    }
        ...
        public DataTable GetContractPayBySupplierId(long orderId, long serviceId, string proc)
        {
            try
            {
