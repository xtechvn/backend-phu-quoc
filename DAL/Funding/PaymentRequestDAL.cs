...
namespace DAL.Funding
{
    public class PaymentRequestDAL : GenericService<PaymentRequest>
    {
        ...
        public DataTable GetListUser(int userId, string proc)
        {
            try
            {
                SqlParameter[] objParam = new SqlParameter[1];
                objParam[0] = new SqlParameter("@PaymentCode", userId);
                return _DbWorker.GetDataTable(proc, objParam);
            }
            catch (Exception ex)
            {
                LogHelper.InsertLogTelegram("GetListUser - PaymentRequestDAL: " + ex);
            }
            return null;
        }
            ...
            try
            {
                ...
                if (id > 0 && model.PaymentRequestDetails != null)
                {
                    foreach (var item in model.PaymentRequestDetails)
                    {
                        ...
                        objParam_requestDetail[5] = new SqlParameter("@ServiceId", item.ServiceId);
                        objParam_requestDetail[6] = new SqlParameter("@Type", item.ServiceType);
                        objParam_requestDetail[7] = new SqlParameter("@ServiceCode", item.ServiceCode);
        ...
        public PaymentRequestDetail GetByRequestDetailById(long detailId)
        {
            try
            {
                ...
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var contractPays = _DbContext.PaymentRequestDetail.Where(x => x.Id == detailId).ToList();
                    if (contractPays != null)
                    {
                        return contractPays.FirstOrDefault();
                    }
                }
                return null;
            }
        ...
        public int ApproveRequest(PaymentRequest model)
        {
            try
            {
                ...
                objParam[2] = new SqlParameter("@OrderStatus", (int)OrderConstants.ORDER_STATUS.DON_HANG_THANH_CONG);
