...
namespace DAL.Funding
{
    public class DepositHistoryDAL : GenericService<DepositHistory>
    {
        ...
        public List<DepositHistory> GetByClientId(long clientId)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var details = _DbContext.DepositHistory.AsNoTracking().Where(x => x.ClientId == clientId).ToList();
                    if (details != null)
                    {
                        return details;
                    }
            ...
            {
                LogHelper.InsertLogTelegram("GetByClientId - DepositHisotyDAL: " + ex);
                return new List<DepositHistory>();
            }
        }

        public int UnderPayment(string transNo)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var detail = _DbContext.DepositHistory.AsNoTracking().FirstOrDefault(x => x.TransNo == transNo);
                    detail.Status = (int)DepositHistoryConstant.DEPOSIT_STATUS.CHO_DUYET;
                    detail.UpdateLast = DateTime.Now;
            ...
            {
                LogHelper.InsertLogTelegram("UnderPayment - DepositHisotyDAL: " + ex);
                return -1;
            }
        }
