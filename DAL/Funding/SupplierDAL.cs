    ...
    public class SupplierDAL : GenericService<Supplier>
    {
        ...
        public Supplier CheckExistName(int id, string name)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var detail = _DbContext.Supplier.FirstOrDefault(x => x.SupplierId != id && name == x.FullName);
                    return detail;
                }
            }
            catch (Exception ex)
        ...
        public DataTable GetRoomBySupplierId(long supplier_id, int page_index = 1, int page_size = 10)
        {
            try
            {
                SqlParameter[] objParam = new SqlParameter[]
