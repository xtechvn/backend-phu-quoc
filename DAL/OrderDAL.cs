    ...
    public class OrderDAL : GenericService<Order>
    {
        ...
        public async Task<DataTable> GetPagingList(OrderViewSearchModel searchModel, int currentPage, int pageSize, string proc)
        {
            try
            {
                ...
                objParam[6] = new SqlParameter("@UtmSource", searchModel.UtmSource == null ? "" : string.Join(",", searchModel.UtmSource));
                ...
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    ...
                    if (data != null && data.OrderId > 0)
                    {
                        ...
                        if (list_hotel_booking != null && list_hotel_booking.Count > 0)
                        {
                            ...
                            {
                                price += list_hotel_optional.Sum(x => x.TotalAmount);
                            }
                            else
                            {
                        ...
                        if (!status_confirm)
                        {
                          ...
                          || (list_other_booking_all != null && list_other_booking_all.Count > 0 && list_other_booking_all.Any(x => x.Status != (int)ServiceStatus.Payment && x.Status != (int)ServiceStatus.Cancel));
                            ...
                            if (!has_other_than_payment && atleast_has_one && data.OrderStatus == (int)OrderStatus.WAITING_FOR_OPERATOR)
                            {
        ...
        public int UpdateOrder(Order model)
        {
            try
            {
                ...
                if (model.UtmMedium != null)
                {
                    objParam_order[32] = new SqlParameter("@UtmMedium", model.UtmMedium);
                }
                else
                {
                    objParam_order[32] = new SqlParameter("@UtmMedium", DBNull.Value);
                }
                if (model.UtmSource != null)
                {
                    objParam_order[33] = new SqlParameter("@UtmSource", model.UtmSource);
                }
                else
                {
