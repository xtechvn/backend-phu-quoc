...
namespace DAL.Report
{
    public class OperatorReportDAL : GenericService<Entities.Models.Invoice>
    {
        ...
        public DataTable GetWSOperatorReport(WSReportSearchModel searchModel)
        {
            try
            {
                SqlParameter[] objParam = new SqlParameter[5];
                ...
                if (searchModel.ClientIds != null)
                {
                    objParam[2] = new SqlParameter("@ClientIds", searchModel.ClientIds);
                }
                else
                {
                    objParam[2] = new SqlParameter("@ClientIds", DBNull.Value);
                }

                if (searchModel.ServiceType != null)
                {
                    objParam[3] = new SqlParameter("@ServiceType", searchModel.ServiceType);
                }
                else
                {
                    objParam[3] = new SqlParameter("@ServiceType", DBNull.Value);
                }

                objParam[4] = new SqlParameter("@VAT", searchModel.VAT);

                return _DbWorker.GetDataTable(StoreProcedureConstant.Report_ToTalRevenueOfSeaSports, objParam);
            }
            catch (Exception ex)
            {
                LogHelper.InsertLogTelegram("GetWSOperatorReport - OperatorReportDAL: " + ex);
            }
            return null;
        }
    }
