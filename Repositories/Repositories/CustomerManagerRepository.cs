...
namespace Repositories.Repositories
{
    public class CustomerManagerRepository : ICustomerManagerRepository
    {
        ...
        public async Task<int> SetUpClientAsync(CustomerManagerView model)
        {
            try
            {
                if (model.Id == 0)
                {
                    ...
                    var data2 =await _ClientDAL.GetClientDetail((int)CreateClient);
                    ...
                    if (CreateClient > 0 && CreateClient != 2)
                    {
                        if (data2 != null)
                        {
                            ...
                            if (Account_Client.ClientType == ClientType.kl)
                            {
                                Account_Client.Status = 0;
                            }
        ...
        public async Task<CustomerManagerViewModel> GetDetailClient(long ClientId)
        {
            ...
            try
            {
                ...
                var listPayment = await _contractPayRepository.GetContractPayByClientId(ClientId);

                if (listPayment != null && listPayment.Count > 0)
                {
                    double sum = listPayment.Where(s => s.DebtStatus == (int)PaymentStatus.PAID).Sum(s => s.Amount);
                    model.sum_payment = sum;
                }
        ...
        public int ResetStatusAc(long clientId, long Status)
        {
            try
            {
