...
namespace Repositories.Repositories
{
    public class OtherBookingRepository : IOtherBookingRepository
    {
        ...
        public async Task<OtherBooking> GetWaterSportById(long booking_id)
        {
            return await otherBookingDAL.GetWaterSportBookingById(booking_id);
        }
        public async Task<long> SummitWaterSportBooking(OrderManualWaterSportBookingServiceSummitModel data, int user_summit)
        {
            try
            {
                OrderManualOtherBookingServiceSummitSQLModel summit = new OrderManualOtherBookingServiceSummitSQLModel();
                ...
                if (data.id > 0)
                {
                    summit.booking = await otherBookingDAL.GetWaterSportBookingById(data.id);
                }
                else
                {
                    summit.booking = new OtherBooking();
                ...
                {
                    summit.booking.ServiceType = allcode_ws_type.CodeValue;
                }
                ...
                var return_value = await otherBookingDAL.CreateOrUpdateOtherBooking(summit);
                if (data.service_code == null || (!data.service_code.ToUpper().Contains("WS") && summit.booking.Id > 0))
                {
                    string s_format = string.Format(String.Format("{0,5:00000}", summit.booking.Id));
                    summit.booking.ServiceCode = "WS" + s_format;
                    await otherBookingDAL.UpdateOtherBooking(summit.booking);
            ...
            {
                LogHelper.InsertLogTelegram("SummitWaterSportBooking - OtherBookingRepository: " + ex);
            }
            return -1;
        ...
        {
            return await otherBookingDAL.GetWaterSportPackagesByBookingId(booking_id);
        }
        public async Task<List<OrderWSExcelSuccessModel>> SummitWSOrder(List<OrderWSExcelSQLModel> summit_model)
        {
            List<OrderWSExcelSuccessModel> success_model = new List<OrderWSExcelSuccessModel>();
            try
            {
                if(summit_model != null && summit_model.Count > 0)
                {
                    foreach(var order in summit_model)
                    {
                        var contact_client_id =  contactClientDAL.CreateContactClients(order.ContactClient, (long)order.order.AccountClientId);
                        order.order.ContactClientId = contact_client_id;
                        order.order.OrderNo = await _identifierServiceRepository.buildOrderNoManual(Convert.ToInt32(_configuration["CompanyType"]==null?"0": _configuration["CompanyType"]));
                        var start_date = order.booking.First().detail.StartDate;
                        var end_date = order.booking.First().detail.EndDate;
                        bool is_first = false;
                        foreach (var booking in order.booking)
                        {
                            if (!is_first)
                            {
                                is_first = true;
                            ...
                            if (booking.detail.StartDate < start_date)
                            {
                                start_date = booking.detail.StartDate;
                            }
                            if(booking.detail.EndDate > end_date)
                            {
                                end_date = booking.detail.EndDate;
                            }
                        }
                        order.order.StartDate = start_date;
                        order.order.EndDate = end_date;
                        var order_id = await orderDAL.CreateOrder(order.order);
                        if (order_id > 0)
                        {
                            order.order.OrderId = order_id;
                            foreach(var booking in order.booking)
                            {
                                booking.detail.OrderId = order_id;
                               
                                var booking_id = await otherBookingDAL.CreateOtherBooking(booking.detail);
                               
                                if (booking_id > 0)
                                {
                                    string s_format = string.Format(String.Format("{0,5:00000}", booking.detail.Id));
                                    booking.detail.ServiceCode = "WS" + s_format;
                                    await otherBookingDAL.UpdateOtherBooking(booking.detail);
                                    foreach (var packages in booking.packages)
                                    {
                                        packages.BookingId = booking_id;
                                        await  otherBookingPackagesDAL.CreateOtherBookingPackages(packages);
                                       
                                    }
                                }
                            }
                            
                            success_model.Add(new OrderWSExcelSuccessModel()
                            {
                                amount = Convert.ToDouble(order.order.Amount),
                                client_id = order.order.ClientId.ToString(),
                                conf_no = order.booking[0].detail.ConfNo,
                                order_no = order.order.OrderNo,
                                order_id = order.order.OrderId,
                                room_no = order.booking[0].detail.RoomNo,
                                serial_no = order.booking[0].detail.SerialNo,
                                used_date = order.booking[0].detail.StartDate
                            });
                        }
            ...
            {
                LogHelper.InsertLogTelegram("SummitWSOrder - OtherBookingRepository: " + ex);
            }
