    ...
    public class FlyBookingDetailRepository : IFlyBookingDetailRepository
    {
        ...
        public async Task<long> SummitFlyBookingServiceData(OrderManualFlyBookingServiceSummitModel data, int is_client_debt)
        {
            try
            {
                ...
                //-- Calucate Profit:
                var ticket_profit = (adt_object == null || adt_object.profit <= 0 ? 0 : adt_object.profit) + (chd_object == null || chd_object.profit <= 0 ? 0 : chd_object.profit) + (inf_object == null || inf_object.profit <= 0 ? 0 : inf_object.profit);
                var ticket_profit_adt = (adt_object == null || adt_object.profit <= 0 ? 0 : adt_object.profit);
                var ticket_profit_chd = (chd_object == null || chd_object.profit <= 0 ? 0 : chd_object.profit);
                var ticket_profit_inf = (inf_object == null || inf_object.profit <= 0 ? 0 : inf_object.profit);
                ...
                if (data.go != null)
                {
                    ...
                    var route = new FlyBookingDetail()
                    {
                        ...
                        AmountAdt = data.amount_adt > 0 ? Math.Round((data.amount_adt - ticket_profit_adt) / (double)multi, 0) : 0,
                        AmountChd = data.amount_chd > 0 ? Math.Round((data.amount_chd - ticket_profit_chd) / (double)multi, 0) : 0,
                        AmountInf = data.amount_inf > 0 ? Math.Round((data.amount_inf - ticket_profit_inf) / (double)multi, 0) : 0,
                        ...
                        FareAdt = data.amount_adt > 0 ? Math.Round((data.amount_adt - ticket_profit_adt) / (double)multi / (double)(adult_number), 0) : 0,
                        FareChd = data.amount_chd > 0 ? Math.Round((data.amount_chd - ticket_profit_chd) / (double)multi / (double)(child_number), 0) : 0,
                        FareInf = data.amount_inf > 0 ? Math.Round((data.amount_inf - ticket_profit_inf) / (double)multi / (double)(infant_number), 0) : 0,
                        FeeAdt = 0,
                        FeeChd = 0,
                        FeeInf = 0,
                        ...
                        UpdatedBy = data.user_summit,
                        UpdatedDate = DateTime.Now,
                        PriceAdt = data.amount_adt > 0 ? Math.Round((data.amount_adt - ticket_profit_adt) / (double)multi, 0) : 0,
                        PriceChd = data.amount_chd > 0 ? Math.Round((data.amount_chd - ticket_profit_chd) / (double)multi, 0) : 0,
                        PriceInf = data.amount_inf > 0 ? Math.Round((data.amount_inf - ticket_profit_inf) / (double)multi, 0) : 0,
