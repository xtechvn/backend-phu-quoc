...
namespace Repositories.Repositories
{
    public class PaymentRequestRepository : IPaymentRequestRepository
    {
        ...
        public List<int> GetListUserRole(int userId)
        {
            var listUserId = new List<int>();
            try
            {
                var userIds = userDAL.GetListUserByUserId(userId);
                listUserId = userIds.Split(",").Select(n => int.Parse(n)).ToList();
            }
            catch (Exception ex)
            {
                LogHelper.InsertLogTelegram("GetListUserRole - PaymentRequestRepository: " + ex);
            }
            return listUserId;
        }
        ...
        public List<PaymentRequestViewModel> GetByClientId(long clientId, int paymentVoucherId = 0)
        {
            try
            {
                var listPaymentRequest = paymentRequestDAL.GetServiceListByClientId(clientId,
                    ...
                    .Where(n => n.Status == (int)PAYMENT_REQUEST_STATUS.CHO_CHI).ToList(); ;
