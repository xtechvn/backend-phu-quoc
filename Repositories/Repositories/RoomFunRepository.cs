                    ...
                    foreach (var contract in contract_list)
                    {
                        if (contract != null && contract.packages_list != null && contract.packages_list.Count > 0)
                            foreach (var package in contract.packages_list)
                            {
                                ...
                                {
                                    result.Add(new HotelPricePolicyDetail()
                                    {
                                        ProductService = new ProductRoomServiceCMSViewModel()
                                        {
                                            AllotmentsId = contract.contract.AllotmentId,
                                            BasePrice = (double)room.Price,
                                            CampaignId = campaign_id,
                                            ContractNo = contract.contract.AllotmentName,
                                            PackageId = package.package.PackageId,
                                            PackageName = package.package.Code,
                                            GroupProviderType = contract.contract.GroupProviderType,
                                            ProviderId = contract.contract.HotelId,
                                            ProviderName = "",
                                            RoomId = room.RoomId,
