...
var _add_payment_request = {
    ...
    SetServiceData: function () {
        ...
        supplierId = $('#supplierId_service').val()
        ...
        if (serviceId !== undefined && serviceId !== null && parseInt(serviceId) !== 0) {
            ...
            if (serviceType == 5 || serviceType == 9 || serviceType == 1 || serviceType == 3) // dvu Tour hoặc dvu Khác hoặc dvu khách sạn hoặc dv vé máy bay
            {
                ...
                if (serviceType == 5)// dvu Tour
                    _add_payment_request.GetTourPackageOption(serviceId);
                if (serviceType == 9)// dvu Khác
                    _add_payment_request.GetOtherPackageOption(serviceId);
                if (serviceType == 1)// dvu khách sạn 
                    _add_payment_request.GetHotelPackageOption(serviceId);
                if (serviceType == 3)// dvu vé máy bay
                    _add_payment_request.GetFlyBookingPackagesOptional(serviceId);
                $('#amount').val(0)
            }
    ...
    GetTourPackageOption: function (serviceId) {
    ...
    GetOtherPackageOption: function (serviceId) {
    ...
    GetHotelPackageOption: function (serviceId) {
        _global_function.AddLoading()
        $.ajax({
    ...
    GetFlyBookingPackagesOptional: function (serviceId) {
        _global_function.AddLoading()
        $.ajax({
    ...
    Validate: function () {
        ...
        if (!isEdit && serviceId !== undefined && serviceId !== null && parseInt(serviceId) !== 0 && parseInt(payment_request_type) === 1) {
            ...
            if ((serviceType == 5 || serviceType == 9 || serviceType == 1 || serviceType == 3) &&
                ...
                if (amount > (amount_supplier - amount_supplier_remain)) {
                ...
                if (amount > (amount_service - totalPayment_service)) {
