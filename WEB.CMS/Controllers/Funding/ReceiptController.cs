...
namespace WEB.Adavigo.CMS.Controllers
{
    ...
    public class ReceiptController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            ViewBag.isQcEnvironment = false;
            if (_configuration["Config:On_QC_Environment"] == "1")
            {
                ViewBag.isQcEnvironment = true;
            }
            return View();
        }
        ...
        public async Task<IActionResult> GetListBankAccountAdavigo()
        {
            try
            {
                var listBankAccount = _allCodeRepository.GetBankingAccounts().Where(n => n.SupplierId ==
                    (long)config.SUPPLIERID_ADAVIGO).ToList();
                return Ok(new
                {
                    isSuccess = true,
