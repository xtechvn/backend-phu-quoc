...
namespace WEB.Adavigo.CMS.Controllers.Order
{
    ...
    public partial class OrderController : Controller
    {
        ...
        public async Task<IActionResult> AddFlyBookingService(long order_id, string group_fly)
        {
            ...
            {
                LogHelper.InsertLogTelegram("AddFlyBookingService - OrderManualController: " + ex);
            }
            return View();
        }
        [HttpPost]
        ...
        public async Task<IActionResult> ChangeOrderSaler(long? order_id,int saleid,string OrderNo)
        {
            ...
            try
            {
                ...
                var SendMessage = apiService.SendMessage( _UserId.ToString(), ((int)ModuleType.DON_HANG).ToString(), ((int)ActionType.NHAN_TRIEN_KHAI).ToString(), OrderNo, link, current_user.Role);
                return Ok(new
                {
                    status = (int)ResponseType.SUCCESS,
        ...
        [HttpPost]
        public async Task<IActionResult> AddWaterSportService(long order_id, long booking_id)
        {
            try
            {
                ViewBag.User = null;
                ...
                if (order_id > 0)
                {
                    ...
                    if (booking_id > 0)
                    {
                        var booking = await _otherBookingRepository.GetWaterSportById(booking_id);
                        bool is_allow_to_edit = false;
                        if (booking != null && booking.Id > 0)
                        {
                            ViewBag.Booking = booking;
                            var user_by_booking = await _userRepository.GetById(((int)booking.OperatorId));
                            if (user_by_booking != null && user_by_booking.Id > 0)
                            {
                                ViewBag.User = user_by_booking;
                            }
            ...
            {
                LogHelper.InsertLogTelegram("AddWaterSportService - OrderManualController: " + ex.ToString());
            }
            return View();
        }
        [HttpPost]
        public async Task<IActionResult> AddWaterSportServicePackages(long booking_id)
        {
            ...
            {
                LogHelper.InsertLogTelegram("AddWaterSportServicePackages - OrderManualController: " + ex.ToString());
            }
            return View();
        }
        [HttpGet]
        ...
        [HttpPost]
        public async Task<IActionResult> SummitwatersportServicePackages(OrderManualWaterSportBookingServiceSummitModel data)
        {
            try
            {
                int _UserId = 0;
