...
namespace WEB.Adavigo.CMS.Controllers.Invoice
{
    ...
    public class InvoiceRequestController : Controller
    {
        ...
        public async Task<IActionResult> Detail(int invoiceRequestId)
        {
            ...
            var current_user = _ManagementUser.GetCurrentUser();
            ViewBag.DUYET_YEU_CAU_XUAT_HOA_DON = 0;
            if (current_user != null && !string.IsNullOrEmpty(current_user.Role))
            {
                ...
                foreach (var item in listRole)
                {
                    ...
                    {
                        ViewBag.DUYET_YEU_CAU_XUAT_HOA_DON = 1;
                        break;
                    }
                }
            }
        ...
        public async Task<IActionResult> AddJson(InvoiceRequestViewModel model)
        {
            try
            {
                ...
                {
                    var current_user = _ManagementUser.GetCurrentUser();
                    var invoiceRequestHistory = new InvoiceRequestHistoryViewModel();
                    invoiceRequestHistory.InvoiceRequestId = result;
                    invoiceRequestHistory.CreatedBy = userLogin;
                    invoiceRequestHistory.CreatedDate = DateTime.Now;
                    invoiceRequestHistory.Actioin = "Gửi đi";
                    invoiceRequestHistory.UserCreateName = current_user.Name;
                    ...
                    j_param = new Dictionary<string, object>
                        {
                             ...
                             {"KeyID",result.ToString()},
                             {"Type", LogType.ACTIVITY},
                             {"CompanyType", LogHelper.CompanyTypeInt},
                             {"ObjectType", ObjectType.INVOICE_REQUEST},
                             {"Log", JsonConvert.SerializeObject(invoiceRequestHistory)},
                        };
                    ...
                    if (output.status != 0)
                        LogHelper.InsertLogTelegram("AddJson - InvoiceRequestController. Lỗi không push được log . " + contents);
                }
        ...
        public async Task<IActionResult> Update(InvoiceRequestViewModel model)
        {
            try
            {
                ...
                if (model.isSend == 1)
                {
                    var current_user = _ManagementUser.GetCurrentUser();
                    var invoiceRequestHistory = new InvoiceRequestHistoryViewModel();
                    invoiceRequestHistory.InvoiceRequestId = (int)model.Id;
                    invoiceRequestHistory.CreatedBy = userLogin;
                    invoiceRequestHistory.CreatedDate = DateTime.Now;
                    invoiceRequestHistory.Actioin = "Gửi đi";
                    invoiceRequestHistory.UserCreateName = current_user.Name;
                    ...
                    if (output.status != 0)
                        LogHelper.InsertLogTelegram("Update - InvoiceRequestController. Lỗi không push được log . " + contents);
                }
                return Ok(new
                {
                    isSuccess = true,
        ...
        [HttpPost]
        public async Task<IActionResult> Approve(int invoiceRequestId, int status)
        {
            try
            {
                var userLogin = 0;
                ...
                invoiceRequestHistory.CreatedBy = userLogin;
                invoiceRequestHistory.CreatedDate = DateTime.Now;
                invoiceRequestHistory.Actioin = "Duyệt yêu cầu";
                invoiceRequestHistory.UserCreateName = current_user.Name;
                var httpClient = new HttpClient();
                var apiPrefix = ReadFile.LoadConfig().API_URL + ReadFile.LoadConfig().API_INSERT_LOG_ACTIVITY;
                var key_token_api = ReadFile.LoadConfig().KEY_TOKEN_API_MANUAL;
                var j_param = new Dictionary<string, object>
                        {
                             {"SourceID", (int)LogSource.BACKEND},
                             {"KeyID",invoiceRequestId.ToString()},
                             {"Type", LogType.ACTIVITY},
                             {"CompanyType", LogHelper.CompanyTypeInt},
                             {"ObjectType", ObjectType.INVOICE_REQUEST},
                             {"Log", JsonConvert.SerializeObject(invoiceRequestHistory)},
                ...
                var output = JsonConvert.DeserializeObject<OutputAPI>(contents);
                if (output.status != 0)
                    LogHelper.InsertLogTelegram("Approve - InvoiceRequestController. Lỗi không push được log . " + contents);
                return Ok(new
                {
                    isSuccess = true,
        ...
        public async Task<IActionResult> RejectRequest(int invoiceRequestId, string note)
        {
            try
            {
                ...
                invoiceRequestHistory.CreatedBy = userLogin;
                invoiceRequestHistory.CreatedDate = DateTime.Now;
                invoiceRequestHistory.Actioin = "Từ chối";
                invoiceRequestHistory.UserCreateName = current_user.Name;
                var httpClient = new HttpClient();
                var apiPrefix = ReadFile.LoadConfig().API_URL + ReadFile.LoadConfig().API_INSERT_LOG_ACTIVITY;
                var key_token_api = ReadFile.LoadConfig().KEY_TOKEN_API_MANUAL;
                ...
                var output = JsonConvert.DeserializeObject<OutputAPI>(contents);
                if (output.status != 0)
                    LogHelper.InsertLogTelegram("RejectRequest - InvoiceRequestController. Lỗi không push được log . " + contents);
                return Ok(new
                {
                    isSuccess = true,
        ...
        public IActionResult History(int requestId)
        {
            ...
            var listRequestHistory = new List<InvoiceRequestHistoryViewModel>();
            FilterMongoService filterMongoService = new FilterMongoService();
            var listDebtBrickLog = filterMongoService.SearchHistoryBackend(requestId, new List<string> { ObjectType.INVOICE_REQUEST });
            foreach (var item in listDebtBrickLog)
            {
                var model = JsonConvert.DeserializeObject<InvoiceRequestHistoryViewModel>(item.Log);
                listRequestHistory.Add(model);
            }
            ViewBag.listRequestHistory = listRequestHistory;
            return PartialView();
        }
