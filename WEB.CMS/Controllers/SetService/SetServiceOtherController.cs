            ...
            try
            {
                ...
                if (current_user != null)
                {
                    if (current_user.Role != "")
                    {
                        ...
                        {
                            var others_type = _allCodeRepository.GetListByType(AllCodeType.SERVICE_TYPE_OTHER);
                            var others_main_type = _allCodeRepository.GetListByType(AllCodeType.SERVICE_TYPE_OTHER_MAIN);
                            var other_main_type_value = others_main_type.Select(x => x.CodeValue);
                            if (other_main_type_value != null && other_main_type_value.Count() > 0)
                            {
                                searchModel.ServiceType = others_type.Where(x => !other_main_type_value.Contains(x.CodeValue)).Select(x => (int)x.CodeValue).ToList();
                            }
                        }
